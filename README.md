...
### 1. 设备列表页
- 新增主机：手动录入新主机信息
- 导入巡检文件：支持导入CSV/Excel格式的巡检报告，可同时导入硬件画像信息
- 按部门筛选：下拉选择部门过滤设备列表
- 按状态筛选：在线/离线/退役等状态过滤
...
- 重复主机名：网络中存在相同主机名的设备
- 保修临期提醒：保修期在30天内即将到期
- **自动检测**：新增、导入或修改设备时自动刷新异常结果
- **状态持久化**：解决或忽略后状态自动保存

### 5. 批量操作窗口
- 生成盘点表：导出Excel格式的资产盘点报表（包含硬件配置和标签信息）
- 打印资产贴纸：生成可打印的资产标签（含条形码）
- 记录移交：记录设备移交历史
- 归档退役设备：将退役设备移至归档库
- 保存历次快照：批量保存设备配置快照，**显示每台设备的详细结果**
...
3. 系统将自动解析并导入设备信息及硬件画像
4. 自动触发异常检测，刷新异常列表

**支持的导入字段：**
- 基础信息：hostname, ip_address, mac_address, serial_number, department, status
- 硬件信息：processor, memory, disk, graphics, os_info, software_list
- 标签信息：owner, location, warranty_end
