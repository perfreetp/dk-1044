...
- **智能去重**：解决或忽略的异常不会被重复检测，只有真正发生新的异常时才标记为待处理
- **状态持久化**：解决或忽略后状态自动保存

### 5. 批量操作窗口
- 生成盘点表：导出标准.xlsx格式的资产盘点报表（包含硬件配置和标签信息）
...
├── tailwind.config.js
├── index.html
├── sample-import-data.csv       # CSV示例导入数据
├── sample-import-data.xlsx      # Excel示例导入数据
├── sample-import-template.csv   # CSV导入模板
├── src/
│   ├── main.tsx
│   ├── App.tsx
│   ├── index.css
...
- 硬件信息：processor, memory, disk, graphics, os_info, network_info, software_list
- 标签信息：owner, location, warranty_end, purpose
...
### 智能去重
- 已解决/忽略的异常不会被重复检测
- 只有设备情况真正发生变化时才产生新的待处理记录
- 通过异常唯一标识（设备ID+异常类型+描述）进行比对
